...
const baseImage = env.files[0]; //.find(file => /\.(png|jpg|jpeg)$/i.test(file.filename));
const baseImageBuffer = baseImage ? readFileSync(baseImage.filename) : undefined;

env.output.appendContent(`Base image found: ${baseImage ? baseImage.filename : 'None'}`);
env.output.appendContent(`Base image buffer: ${baseImageBuffer ? 'Loaded' : 'Not provided'}`);
...
${baseImage ? `The banner should be based on the provided image.` : ''}
